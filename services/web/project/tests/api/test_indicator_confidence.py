...
def test_create_duplicate(client):
    ...
    assert response['message'] == 'Indicator confidence already exists'
...
def test_read_nonexistent_id(client):
    ...
    request = client.get('/api/indicators/confidence/100000')
    response = json.loads(request.data.decode())
    assert request.status_code == 404
    assert response['message'] == 'Indicator confidence ID not found'
...
def test_read_invalid_api_key(app, client):
    ...
    request = client.get('/api/indicators/confidence/1?apikey={}'.format(TEST_INVALID_APIKEY))
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['message'] == 'API user does not exist'
...
def test_read_inactive_api_key(app, client):
    ...
    request = client.get('/api/indicators/confidence/1?apikey={}'.format(TEST_INACTIVE_APIKEY))
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['message'] == 'API user is not active'
...
def test_read_invalid_role(app, client):
    ...
    request = client.get('/api/indicators/confidence/1?apikey={}'.format(TEST_ANALYST_APIKEY))
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['message'] == 'Insufficient privileges'
...
def test_update_missing_api_key(app, client):
    ...
    data = {'value': 'asdf'}
    request = client.put('/api/indicators/confidence/1', data=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['message'] == 'Bad or missing API key'
...
def test_delete_nonexistent_id(client):
    ...
    request = client.delete('/api/indicators/confidence/100000')
    response = json.loads(request.data.decode())
    assert request.status_code == 404
    assert response['message'] == 'Indicator confidence ID not found'
...
def test_delete_invalid_api_key(app, client):
    ...
    request = client.delete('/api/indicators/confidence/1', data=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['message'] == 'API user does not exist'
...
def test_delete_foreign_key(client):
    ...
    confidence_request, confidence_response = create_indicator_confidence(client, 'LOW')
    indicator_request, indicator_response = create_indicator(client, 'IP', '127.0.0.1', 'analyst', confidence='LOW')
    assert confidence_request.status_code == 201
    assert indicator_request.status_code == 201

    request = client.delete('/api/indicators/confidence/{}'.format(confidence_response['id']))
    response = json.loads(request.data.decode())
    assert request.status_code == 409
    assert response['message'] == 'Unable to delete indicator confidence due to foreign key constraints'
...
def test_delete(client):
    ...
    request = client.delete('/api/indicators/confidence/{}'.format(_id))
