...
def test_create_duplicate(client):
    ...
    assert response['msg'] == 'Event status already exists'
...
def test_read_nonexistent_id(client):
    ...
    assert response['msg'] == 'Event status ID not found'
...
def test_update_schema(client):
    ...
    # Invalid JSON
    data = {}
    request = client.put('/api/events/status/1', json=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 400
    assert response['msg'] == 'Request must include valid JSON'
...
def test_update_nonexistent_id(client):
    ...
    data = {'value': 'asdf'}
    request = client.put('/api/events/status/100000', json=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 404
    assert response['msg'] == 'Event status ID not found'
...
def test_update_duplicate(client):
    ...
    data = {'value': 'asdf'}
    request = client.put('/api/events/status/{}'.format(_id), json=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 409
    assert response['msg'] == 'Event status already exists'
...
def test_delete_foreign_key(client):
    ...
    response = json.loads(request.data.decode())
    assert request.status_code == 409
    assert response['msg'] == 'Unable to delete event status due to foreign key constraints'
