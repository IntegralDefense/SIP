...
def test_create_duplicate(client):
    ...
    assert response['msg'] == 'Indicator type already exists'
...
def test_create_missing_api_key(app, client):
    ...
    request = client.post('/api/indicators/type')
    response = json.loads(request.data.decode())
...
def test_create_invalid_api_key(app, client):
    ...
    request = client.post('/api/indicators/type', headers=headers)
    response = json.loads(request.data.decode())
    assert request.status_code == 401
...
def test_read_nonexistent_id(client):
    ...
    request = client.get('/api/indicators/type/100000')
    response = json.loads(request.data.decode())
    assert request.status_code == 404
    assert response['msg'] == 'Indicator type ID not found'
...
def test_read_missing_api_key(app, client):
    ...
    request = client.get('/api/indicators/type/1')
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['msg'] == 'Bad or missing API key'
...
def test_read_invalid_api_key(app, client):
    ...
    headers = {'Authorization': 'Apikey ' + TEST_INVALID_APIKEY}
    request = client.get('/api/indicators/type/1', headers=headers)
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['msg'] == 'API user does not exist'
...
def test_read_all_values(client):
    ...
    request = client.get('/api/indicators/type')
    response = json.loads(request.data.decode())
...
def test_read_by_id(client):
    ...
    request = client.get('/api/indicators/type/{}'.format(_id))
    response = json.loads(request.data.decode())
    assert request.status_code == 200
...
def test_update_schema(client):
    ...
    # Invalid JSON
    data = {}
    request = client.put('/api/indicators/type/1', json=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 400
    assert response['msg'] == 'Request must include valid JSON'
...
def test_update_nonexistent_id(client):
    ...
    data = {'value': 'asdf'}
    request = client.put('/api/indicators/type/100000', json=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 404
    assert response['msg'] == 'Indicator type ID not found'
...
def test_update_duplicate(client):
    ...
    data = {'value': 'asdf'}
    request = client.put('/api/indicators/type/{}'.format(_id), json=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 409
    assert response['msg'] == 'Indicator type already exists'
...
def test_update_missing_api_key(app, client):
    ...
    request = client.put('/api/indicators/type/1')
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['msg'] == 'Bad or missing API key'
...
def test_update_invalid_api_key(app, client):
    ...
    headers = {'Authorization': 'Apikey ' + TEST_INVALID_APIKEY}
    request = client.put('/api/indicators/type/1', headers=headers)
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['msg'] == 'API user does not exist'
...
def test_delete_nonexistent_id(client):
    ...
    request = client.delete('/api/indicators/type/100000')
    response = json.loads(request.data.decode())
    assert request.status_code == 404
    assert response['msg'] == 'Indicator type ID not found'
...
def test_delete_missing_api_key(app, client):
    ...
    request = client.delete('/api/indicators/type/1')
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['msg'] == 'Bad or missing API key'
...
def test_delete_invalid_api_key(app, client):
    ...
    headers = {'Authorization': 'Apikey ' + TEST_INVALID_APIKEY}
    request = client.delete('/api/indicators/type/1', headers=headers)
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['msg'] == 'API user does not exist'
...
def test_delete_foreign_key(client):
    ...
    response = json.loads(request.data.decode())
    assert request.status_code == 409
    assert response['msg'] == 'Unable to delete indicator type due to foreign key constraints'
...
def test_delete(client):
    ...
    request = client.delete('/api/indicators/type/{}'.format(_id))
