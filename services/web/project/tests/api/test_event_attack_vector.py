...
def test_create_duplicate(client):
    ...
    assert response['msg'] == 'Event attack vector already exists'
...
def test_create_missing_token(app, client):
    ...
    request = client.post('/api/events/attackvector')
    response = json.loads(request.data.decode())
...
def test_create_invalid_role(app, client):
    ...
    request = client.post('/api/events/attackvector', headers=headers)
    response = json.loads(request.data.decode())
    assert request.status_code == 401
...
def test_read_nonexistent_id(client):
    ...
    request = client.get('/api/events/attackvector/100000')
    response = json.loads(request.data.decode())
    assert request.status_code == 404
    assert response['msg'] == 'Event attack vector ID not found'
...
def test_read_missing_token(app, client):
    ...
    request = client.get('/api/events/attackvector/1')
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['msg'] == 'Missing Authorization Header'
...
def test_read_invalid_role(app, client):
    ...
    access_token, refresh_token = obtain_token(client, 'analyst', 'analyst')
    headers = create_auth_header(access_token)
    request = client.get('/api/events/attackvector/1', headers=headers)
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['msg'] == 'user_does_not_have_this_role role required'
...
def test_read_all_values(client):
    ...
    request = client.get('/api/events/attackvector')
    response = json.loads(request.data.decode())
...
def test_read_by_id(client):
    ...
    request = client.get('/api/events/attackvector/{}'.format(_id))
    response = json.loads(request.data.decode())
    assert request.status_code == 200
...
def test_update_schema(client):
    ...
    # Invalid JSON
    data = {}
    request = client.put('/api/events/attackvector/1', json=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 400
    assert response['msg'] == 'Request must include valid JSON'
...
def test_update_nonexistent_id(client):
    ...
    data = {'value': 'asdf'}
    request = client.put('/api/events/attackvector/100000', json=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 404
    assert response['msg'] == 'Event attack vector ID not found'
...
def test_update_duplicate(client):
    ...
    data = {'value': 'asdf'}
    request = client.put('/api/events/attackvector/{}'.format(_id), json=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 409
    assert response['msg'] == 'Event attack vector already exists'
...
def test_update_missing_token(app, client):
    ...
    request = client.put('/api/events/attackvector/1')
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['msg'] == 'Missing Authorization Header'
...
def test_update_invalid_role(app, client):
    ...
    access_token, refresh_token = obtain_token(client, 'analyst', 'analyst')
    headers = create_auth_header(access_token)
    request = client.put('/api/events/attackvector/1', headers=headers)
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['msg'] == 'user_does_not_have_this_role role required'
...
def test_delete_nonexistent_id(client):
    ...
    request = client.delete('/api/events/attackvector/100000')
    response = json.loads(request.data.decode())
    assert request.status_code == 404
    assert response['msg'] == 'Event attack vector ID not found'
...
def test_delete_missing_token(app, client):
    ...
    request = client.delete('/api/events/attackvector/1')
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['msg'] == 'Missing Authorization Header'
...
def test_delete_invalid_role(app, client):
    ...
    access_token, refresh_token = obtain_token(client, 'analyst', 'analyst')
    headers = create_auth_header(access_token)
    request = client.delete('/api/events/attackvector/1', headers=headers)
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['msg'] == 'user_does_not_have_this_role role required'
...
def test_delete_foreign_key(client):
    ...
    event_request, event_response = create_event(client, 'test_event', 'analyst', attack_vectors=['Webmail'])
    ...
    response = json.loads(request.data.decode())
    assert request.status_code == 409
    assert response['msg'] == 'Unable to delete event attack vector due to foreign key constraints'
...
def test_delete(client):
    ...
    request = client.delete('/api/events/attackvector/{}'.format(_id))
