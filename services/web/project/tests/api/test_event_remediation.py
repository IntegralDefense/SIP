...
def test_create_missing_parameter(client):
    ...
    request = client.post('/api/events/remediation')
...
def test_create_duplicate(client):
    ...
    assert response['message'] == 'Event remediation already exists'
...
def test_read_nonexistent_id(client):
    ...
    request = client.get('/api/events/remediation/100000')
    response = json.loads(request.data.decode())
    assert request.status_code == 404
    assert response['message'] == 'Event remediation ID not found'
...
def test_read_missing_api_key(app, client):
    ...
    request = client.get('/api/events/remediation/1')
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['message'] == 'Bad or missing API key'
...
def test_read_invalid_api_key(app, client):
    ...
    request = client.get('/api/events/remediation/1?apikey={}'.format(TEST_INVALID_APIKEY))
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['message'] == 'API user does not exist'
...
def test_read_inactive_api_key(app, client):
    ...
    request = client.get('/api/events/remediation/1?apikey={}'.format(TEST_INACTIVE_APIKEY))
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['message'] == 'API user is not active'
...
def test_read_invalid_role(app, client):
    ...
    request = client.get('/api/events/remediation/1?apikey={}'.format(TEST_ANALYST_APIKEY))
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['message'] == 'Insufficient privileges'
...
def test_read_all_values(client):
    ...
    request = client.get('/api/events/remediation')
    response = json.loads(request.data.decode())
...
def test_read_by_id(client):
    ...
    request = client.get('/api/events/remediation/{}'.format(_id))
    response = json.loads(request.data.decode())
    assert request.status_code == 200
...
def test_update_nonexistent_id(client):
    ...
    data = {'value': 'asdf'}
    request = client.put('/api/events/remediation/100000', data=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 404
    assert response['message'] == 'Event remediation ID not found'
...
def test_update_missing_parameter(client):
    ...
    request = client.put('/api/events/remediation/{}'.format(_id))
    response = json.loads(request.data.decode())
    assert request.status_code == 400
    assert response['message'] == 'Request must include "value"'
...
def test_update_duplicate(client):
    ...
    data = {'value': 'asdf'}
    request = client.put('/api/events/remediation/{}'.format(_id), data=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 409
    assert response['message'] == 'Event remediation already exists'
...
def test_update_missing_api_key(app, client):
    ...
    data = {'value': 'asdf'}
    request = client.put('/api/events/remediation/1', data=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['message'] == 'Bad or missing API key'
...
def test_delete_nonexistent_id(client):
    ...
    request = client.delete('/api/events/remediation/100000')
    response = json.loads(request.data.decode())
    assert request.status_code == 404
    assert response['message'] == 'Event remediation ID not found'
...
def test_delete_missing_api_key(app, client):
    ...
    request = client.delete('/api/events/remediation/1')
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['message'] == 'Bad or missing API key'
...
def test_delete_invalid_api_key(app, client):
    ...
    request = client.delete('/api/events/remediation/1', data=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['message'] == 'API user does not exist'
...
def test_delete_foreign_key(client):
    ...
    remediation_request, remediation_response = create_event_remediation(client, 'REIMAGED')
    event_request, event_response = create_event(client, 'test_event', 'analyst', remediation='REIMAGED')
    assert remediation_request.status_code == 201
    assert event_request.status_code == 201

    request = client.delete('/api/events/remediation/{}'.format(remediation_response['id']))
    response = json.loads(request.data.decode())
    assert request.status_code == 409
    assert response['message'] == 'Unable to delete event remediation due to foreign key constraints'
...
def test_delete(client):
    ...
    request = client.delete('/api/events/remediation/{}'.format(_id))
