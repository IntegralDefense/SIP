...
def test_create_duplicate(client):
    ...
    assert response['msg'] == 'Event disposition already exists'
...
def test_create_invalid_role(app, client):
    ...
    request = client.post('/api/events/disposition', headers=headers)
    response = json.loads(request.data.decode())
    assert request.status_code == 401
...
def test_read_nonexistent_id(client):
    ...
    assert response['msg'] == 'Event disposition ID not found'
...
def test_read_invalid_role(app, client):
    ...
    access_token, refresh_token = obtain_token(client, 'analyst', 'analyst')
    headers = create_auth_header(access_token)
    request = client.get('/api/events/disposition/1', headers=headers)
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['msg'] == 'user_does_not_have_this_role role required'
...
def test_update_schema(client):
    ...
    # Invalid JSON
    data = {}
    request = client.put('/api/events/disposition/1', json=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 400
    assert response['msg'] == 'Request must include valid JSON'
...
def test_update_nonexistent_id(client):
    ...
    data = {'value': 'asdf'}
    request = client.put('/api/events/disposition/100000', json=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 404
    assert response['msg'] == 'Event disposition ID not found'
...
def test_update_duplicate(client):
    ...
    data = {'value': 'asdf'}
    request = client.put('/api/events/disposition/{}'.format(_id), json=data)
    response = json.loads(request.data.decode())
    assert request.status_code == 409
    assert response['msg'] == 'Event disposition already exists'
...
def test_update_missing_token(app, client):
    ...
    request = client.put('/api/events/disposition/1')
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['msg'] == 'Missing Authorization Header'
...
def test_update_invalid_role(app, client):
    ...
    access_token, refresh_token = obtain_token(client, 'analyst', 'analyst')
    headers = create_auth_header(access_token)
    request = client.put('/api/events/disposition/1', headers=headers)
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['msg'] == 'user_does_not_have_this_role role required'
...
def test_delete_invalid_role(app, client):
    ...
    access_token, refresh_token = obtain_token(client, 'analyst', 'analyst')
    headers = create_auth_header(access_token)
    request = client.delete('/api/events/disposition/1', headers=headers)
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['msg'] == 'user_does_not_have_this_role role required'
...
def test_delete_foreign_key(client):
    ...
    disposition_request, disposition_response = create_event_disposition(client, 'DELIVERY')
    event_request, event_response = create_event(client, 'test_event', 'analyst', disposition='DELIVERY')
    assert disposition_request.status_code == 201
    assert event_request.status_code == 201

    request = client.delete('/api/events/disposition/{}'.format(disposition_response['id']))
    response = json.loads(request.data.decode())
    assert request.status_code == 409
    assert response['msg'] == 'Unable to delete event disposition due to foreign key constraints'
