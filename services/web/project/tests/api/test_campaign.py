...
def test_read_inactive_api_key(app, client):
    ...
    request = client.get('/api/campaigns/1?apikey={}'.format(TEST_INACTIVE_APIKEY))
    response = json.loads(request.data.decode())
    assert request.status_code == 401
    assert response['message'] == 'API user is not active'
...
def test_delete_foreign_key_event(client):
    ...
    event_request, event_response = create_event(client, 'test_event', 'analyst', campaign='Derpsters')
